...
def execSQL(s1):
    ...
    if len(s1) > 100:
        cf.logmessage("Executing SQL: {}...".format(
            s1.strip().replace('\n', ' ')[:100]))
    else:
...
def addTable(df, tablename):
    ...
    if len(discarded_cols):
        cf.logmessage("Dropping {} cols from uploaded data as they're not in the DB: {}".format(
            len(discarded_cols), ', '.join(discarded_cols)))
    ...
    try:
        df[sending_cols].to_sql(name=tablename, con=c,
